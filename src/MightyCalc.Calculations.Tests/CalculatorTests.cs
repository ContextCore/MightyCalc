...
namespace MightyCalc.Calculations.Tests
{
    public class CalculatorTests
    {
        ...
        {
            var result = _calculator.Calculate("a+b-1*sqrt(sub(c,2)+3)", new Parameter("a", 1), new Parameter("b", 2),
                new Parameter("c", 3));

            result.FunctionUsages.Should().BeEquivalentTo(new[]
                {"AddChecked", "SubtractChecked", "MultiplyChecked", "sqrt", "sub", "AddChecked"});
        }
        ...
        [InlineData("1+2-1*sqrt(sub(4,2)+6)", "AddChecked", "SubtractChecked", "MultiplyChecked", "sqrt", "sub",
            "AddChecked")]
        [InlineData("Min(5,2)", "Min")] //factorial
        public void
        ...
        [InlineData(120, "fact(cuberoot)", "cuberoot",
            5D)] //factorial with parameter name conflicting with another function
        [InlineData(1, "a", "a", 0.5, "a",
            1D)] //redefining_parameters_calculating_When_calculating_Then_latest_value_is_used
        public void
        ...
        public void Given_expression_with_non_existing_function_When_calculating_Then_error_is_raised()
        {
            Assert.Throws<ParseException>(() =>
                _calculator.Calculate("notExistingFunc(a)", new[] {new Parameter("a", 1)}));
        }
        ...
        {
            Assert.Throws<SpracheCalculator.AddFunctionException>(() =>
                _calculator.AddFunction("test", "Test function", "sub(add(a,b),c)"));
        }
